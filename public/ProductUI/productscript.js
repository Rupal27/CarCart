function refreshList() {
    ...
    $.get('http://localhost:1212/api/products/', (data) => {
      $('#productList').empty()
   if(data.success)
   {
...
function populateDropDown()
{
  ...
  {
    let path='http://localhost:1212/api/products/'+id;
    $.ajax({
      url: path,
      type: 'DELETE',
      success:(data) => {
  ...
  $('#addProductButton').click(() => {
    ...
    $.post(
       'http://localhost:1212/api/products/',newProduct,  
      (data) => {
        if (data.success) {
          refreshList()
